%% ---- Chapter 9: Finite elements for 2D unsteady Navier Stokes equation ----
disp('----------------------------------------------')
...
funMu = @(x,y) 1;
funF1 = @(x,y,t) -2*pi*(x^2*y^2+exp(-y)) * sin(2*pi*t) + (-2*funMu(x,y)*x^2-2*funMu(x,y)*y^2-funMu(x,y)*exp(-y)+pi^2*cos(pi*x)*cos(2*pi*y))*cos(2*pi*t) + cos(2*pi*t)^2 * (2*x*y^2*(x^2*y^2+exp(-y))+(-2/3*x*y^3+2-pi*sin(pi*x))*(2*y*x^2-exp(-y)));
funF2 = @(x,y,t) -2*pi*(-2/3*x*y^3+2-pi*sin(pi*x))*sin(2*pi*t) + (4*funMu(x,y)*x*y - funMu(x,y)*pi^3*sin(pi*x) + 2*pi*(2-pi*sin(pi*x))*sin(2*pi*y))*cos(2*pi*t) + cos(2*pi*t)^2 * ((x^2*y^2+exp(-y))*(-2/3*y^3-pi^2*cos(pi*x))-2*x*y^2*(-2/3*x*y^3+2-pi*sin(pi*x)));

% funF1 = @(x,y,t) -2*pi*(x^2*y^2+exp(-y)) * sin(2*pi*t) + (-2*funMu(x,y)*x^2-2*funMu(x,y)*y^2-funMu(x,y)*exp(-y)+pi^2*cos(pi*x)*cos(2*pi*y))*cos(2*pi*t)+funRefu1_t(x,y,t)*funRefu1_x_t(x,y,t)+funRefu2_t(x,y,t)*funRefu1_y_t(x,y,t);
% funF2 = @(x,y,t) -2*pi*(-2/3*x*y^3+2-pi*sin(pi*x))*sin(2*pi*t) + (4*funMu(x,y)*x*y - funMu(x,y)*pi^3*sin(pi*x) + 2*pi*(2-pi*sin(pi*x))*sin(2*pi*y))*cos(2*pi*t)+funRefu1_t(x,y,t)*funRefu2_x_t(x,y,t)+funRefu2_t(x,y,t)*funRefu2_y_t(x,y,t);
...
solver = UnsteadyNavierStokesTriSolver(meshInfo,uTrialElementInfo,uTestElementInfo,pTrialElementInfo,pTestElementInfo,boundary,uTrialFun,uTestFun,pTrialFun,pTestFun,quad,funMu,funF1,funF2,funBoundu1,funBoundu2);
...
solver.solve(x0,t0,t1,dt,theta,25,1e-3,1e-6);
